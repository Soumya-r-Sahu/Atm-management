#include "admin/admin_operations.h"
#include "admin/admin_auth.h"
#include "common/utils/logger.h"
#include "common/database/database.h"
#include "common/database/card_account_management.h"
#include "common/paths.h"
#include "common/utils/path_manager.h" // For dynamic paths
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
...
void regenerate_card_pin(int card_number) {
    ...
    if (updatePIN(card_number, newPin)) {
        ...
        printf("New PIN: %d\n", newPin);
        writeSecurityLog("ADMIN", "Card PIN regenerated by admin");
    } else {
...
void toggle_card_status(int card_number) {
    ...
    if (isActive) {
        if (blockCard(card_number)) {
            ...
            writeSecurityLog("ADMIN", "Card blocked by admin");
        } else {
        ...
        if (unblockCard(card_number)) {
            ...
            writeSecurityLog("ADMIN", "Card unblocked by admin");
        } else {
...
int update_atm_status(const char* atm_id, const char* new_status) {
    // Using our database connection function for consistency
    void* conn = db_get_connection();
    if (conn == NULL) {
        writeErrorLog("Failed to get database connection for ATM status update");
        return 0;
    }

    // Simulate a successful status update
    char message[100];
    sprintf(message, "Updated ATM %s status to %s", atm_id, new_status);
