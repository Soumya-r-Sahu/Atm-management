...
int main(int argc, char** argv) {
    ...
    writeSecurityLog("ADMIN", "Admin application started");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--test") == 0) {
            testMode = true;
            setTestingMode(1);
            writeSecurityLog("ADMIN", "Test mode enabled");
        }
    }
    ...
    while (attempts < MAX_ATTEMPTS && !authenticated) {
        ...
        if (!authenticated) {
            ...
            writeSecurityLog("ADMIN", "Admin login successful");
        }
    }

    if (!authenticated) {
        printf("\nToo many failed login attempts. Exiting.\n");
        writeSecurityLog("ADMIN", "Login failed after maximum attempts");
        return EXIT_FAILURE;
    }
    ...
    writeSecurityLog("ADMIN", "Admin application shutting down");
    return EXIT_SUCCESS;
}