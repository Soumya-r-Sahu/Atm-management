...
int initialize_configs(void) {
    ...
    if (file) {
        ...
        if (g_config_count > 0) {
            ...
            writeInfoLog("Loaded system configurations");
            return 1;
        }
    }
    ...
    writeSecurityLog("Created default system configurations");
    return 1;
}
...
int save_configs(void) {
    ...
    fclose(file);
    writeSecurityLog("Saved system configurations to file");
    return 1;
}
...
int add_config(const char* config_name, const char* config_value, const char* description) {
    if (g_config_count >= MAX_CONFIGS) {
        writeErrorLog("Cannot add new configuration: maximum limit reached");
        return 0;
    }
    
    // Check if it already exists
    if (find_config_index(config_name) >= 0) {
        writeErrorLog("Configuration already exists");
        return 0;
    }
...
int getConfigValueInt(const char* key) {
    ...
    return atoi(value); // Default value if not found
}

// Backward compatibility version of integer config getter
int get_config_value_int(const char* config_name) {
    const char* value = get_config_value(config_name);
    if (!value) {
        return 0;
    }
    return atoi(value);
...
int setConfigValue(const char* key, const char* value) {
    ...
    writeSecurityLog("CONFIG", logMsg);
