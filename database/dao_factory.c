...
extern DatabaseAccessObject* createMySQLPooledDAO(void);
...
#if USE_MYSQL
    // Use the pool-based DAO implementation
    instance = createMySQLPooledDAO();
    if (instance) {
        writeInfoLog("Created MySQL Pooled DAO implementation");
        return instance;
    } 
    
    // Fall back to the original MySQL implementation if the pooled one fails
